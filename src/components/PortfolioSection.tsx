...
const categories = ["All", "Branding", "Poster Design", "Social Media", "Web Banner", "Advertising", "Packaging"];
    ...
    <section id="portfolio" className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          {categories.map((c) => (
            <button
              key={c}
              onClick={() => setActive(c)}
              className={`px-4 py-1.5 rounded-full text-xs font-mono-label transition-all ${
          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((p) => (
              <motion.div
                ...
                className={`group relative aspect-[4/3] rounded-xl bg-gradient-to-br ${p.color} border border-border overflow-hidden cursor-pointer`}
              >
        ...
        <div className="flex flex-wrap gap-4 mt-10">
          <a href="https://www.behance.net/mratulhasan10" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors">
            <ExternalLink size={14} /> Behance Portfolio
          </a>
          <a href="https://www.facebook.com/ratulhasan.lemon" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors">
