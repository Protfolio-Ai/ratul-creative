...
const fullTitle = "Creative Digital Designer & Tech Specialist";

// Map words that should be primary colored
...
const HeroSection = () => {
  ...
  // Render typed text with colored words
  const renderTypedText = () => {
    const typed = fullTitle.slice(0, displayedLength);
    ...
    words.forEach((word, wi) => {
      ...
      // How much of this word is visible
      ...
      if (visibleChars > 0) {
        ...
        elements.push(
          <span key={wi} className={isPrimary ? "text-primary" : ""}>
            {visiblePart}
          </span>
        );
      }

      // Add space after word if we've typed past it
          ...
          <div className="flex flex-wrap gap-2.5 mt-2">
            {tags.map((tag, i) => (
              <motion.span
                ...
                className="px-4 py-1.5 rounded-full border border-primary/40 text-sm font-semibold text-foreground backdrop-blur-sm bg-background/50 hover:border-primary hover:bg-primary/10 transition-all cursor-default"
              >
                {tag}
              </motion.span>
            ))}
